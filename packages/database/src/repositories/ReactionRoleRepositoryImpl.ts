...
export class ReactionRoleRepositoryImpl implements ReactionRoleRepository {
	...
	public async findByMessage(
		...
		const rows = await this.prisma.reactionRole.findMany({
			where: { guildId, messageId },
		});
		return rows.map(toReactionRoleDto);
	}

	public async findByMessageAndEmoji(
		guildId: string,
		messageId: string,
		emoji: string,
	): Promise<ReactionRoleDto | null> {
	...
	public async deleteByMessageAndEmoji(
		guildId: string,
		messageId: string,
		emoji: string,
	): Promise<void> {
		await this.prisma.reactionRole.deleteMany({
