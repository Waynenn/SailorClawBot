...
import type { ItemRepository, ItemDto, CreateItemDto, SnowflakeId } from '@sailorclawbot/contracts';
...
export class ItemRepositoryImpl implements ItemRepository {
  ...
  public async findByGuild(guildId: SnowflakeId): Promise<ItemDto[]> {
    const rows = await this.db.item.findMany({ where: { guildId }, orderBy: { price: 'asc' } });
  ...
  public async update(
    id: string,
    data: Partial<Pick<ItemDto, 'name' | 'description' | 'price' | 'emoji' | 'type' | 'effect' | 'stock'>>
  ): Promise<ItemDto> {
    try {
