...
import type { TransactionRepository, TransactionDto } from '@sailorclawbot/contracts';
import { ValidationError } from '@sailorclawbot/core';
import { translatePrismaError } from './prisma-errors.js';

function toTransactionDto(row: { id: string; walletId: string; amount: bigint; reason: string; createdAt: Date }): TransactionDto {
  return {
    id: row.id,
