...
before(async () => {
  ...
  await db.guild.create({ data: { id: guildId, name: 'Integration Test Guild' } });
});
...
test('WarningRepository create / findByGuildAndUser / count', async () => {
  await warnings.create({ guildId, userId, reason: 'spam', moderatorId: modId, caseNumber: 1 });
  await warnings.create({ guildId, userId, reason: 'flood', moderatorId: modId, caseNumber: 2 });
...
test('WarningRepository rejects duplicate case number (unique -> ConflictError)', async () => {
  await warnings.create({ guildId, userId, reason: 'a', moderatorId: modId, caseNumber: 7 });
  await assert.rejects(
    () =>
      warnings.create({
        guildId,
...
test('PermissionRepository create / find / update / deleteByGuildAndUser', async () => {
  ...
  const found = await permissions.findByGuildUserPermission(guildId, modId, 'can_moderate');
  ...
  assert.equal(await permissions.findByGuildUserPermission(guildId, modId, 'can_moderate'), null);
});
