...
import type { XpMultiplierRepository, XpMultiplierDto } from '@sailorclawbot/contracts';
...
export class XpMultiplierRepositoryImpl implements XpMultiplierRepository {
  ...
  public async upsert(data: { guildId: string; targetId: string; targetType: 'channel' | 'role'; multiplier: number }): Promise<XpMultiplierDto> {
    try {
      const row = await this.db.xpMultiplier.upsert({
        where: { guildId_targetId_targetType: { guildId: data.guildId, targetId: data.targetId, targetType: data.targetType } },
        create: data,
