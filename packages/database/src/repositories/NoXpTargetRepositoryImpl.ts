...
import type { NoXpTargetRepository, NoXpTargetDto } from '@sailorclawbot/contracts';
...
export class NoXpTargetRepositoryImpl implements NoXpTargetRepository {
  ...
  public async isExcluded(guildId: string, targetId: string): Promise<boolean> {
    const row = await this.db.noXpTarget.findFirst({ where: { guildId, targetId } });
    return row !== null;
  }

  public async add(data: { guildId: string; targetId: string; targetType: 'channel' | 'role' }): Promise<NoXpTargetDto> {
    try {
