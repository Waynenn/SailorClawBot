...
export class GiveawayRepositoryImpl implements GiveawayRepository {
  ...
  public async create(input: Omit<GiveawayDto, 'id' | 'endedAt' | 'participants' | 'winners'>): Promise<GiveawayDto> {
  ...
  public async setMessageId(id: string, messageId: string): Promise<GiveawayDto> {
    const row = await this.prisma.giveaway.update({ where: { id }, data: { messageId } });
    return toGiveawayDto(row);
  }

  public async addParticipant(id: string, userId: string): Promise<GiveawayDto> {
    const giveaway = await this.prisma.giveaway.findUniqueOrThrow({ where: { id } });
    ...
    const row = await this.prisma.giveaway.update({
      where: { id },
      data: { participants: updated as unknown as Parameters<typeof this.prisma.giveaway.update>[0]['data']['participants'] },
    });
    return toGiveawayDto(row);
  }
  ...
  public async end(id: string, winners: string[]): Promise<GiveawayDto> {
    const row = await this.prisma.giveaway.update({
      ...
      data: {
        endedAt: new Date(),
        winners: winners as unknown as Parameters<typeof this.prisma.giveaway.update>[0]['data']['winners'],
      },
    });
    return toGiveawayDto(row);
  }
