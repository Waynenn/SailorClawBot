...
export function toPermissionOverrideDto(
...
export function toGuildSettingsDto(row: GuildSettings): GuildSettingsDto {
	return {
		...
		familyCreationEnabled: row.familyCreationEnabled,
		maxFamilies: row.maxFamilies,
		maxFamilyMembers: row.maxFamilyMembers,
		familyRequireApproval: row.familyRequireApproval,
		familyCreationMode: row.familyCreationMode,
		familyCreationCost: row.familyCreationCost,
		familyNameChangeCost: row.familyNameChangeCost,
		...
		logChannelOverrides:
			(row.logChannelOverrides as unknown as Record<string, string>) ?? {},
...
export function toTwitchSubscriptionDto(
	row: TwitchSubscription,
): TwitchSubscriptionDto {
	return {
		id: row.id,
		guildId: row.guildId,
...
export function toBlackjackSessionDto(
	row: BlackjackSession,
): BlackjackSessionDto {
	return {
		id: row.id,
		guildId: row.guildId,
		...
		bet: row.bet,
		playerCards: (row.playerCards as unknown as BlackjackCard[]) ?? [],
		dealerCards: (row.dealerCards as unknown as BlackjackCard[]) ?? [],
		deck: (row.deck as unknown as BlackjackCard[]) ?? [],
		createdAt: row.createdAt,
	};
}
