...
export class StarboardEntryRepositoryImpl implements StarboardEntryRepository {
  ...
  public async findByOriginalMessage(guildId: string, originalMsgId: string): Promise<StarboardEntryDto | null> {
  ...
  public async create(input: Omit<StarboardEntryDto, 'id'>): Promise<StarboardEntryDto> {
    const row = await this.prisma.starboardEntry.create({ data: input });
    return toStarboardEntryDto(row);
  }

  public async updateStarCount(guildId: string, originalMsgId: string, starCount: number): Promise<StarboardEntryDto> {
