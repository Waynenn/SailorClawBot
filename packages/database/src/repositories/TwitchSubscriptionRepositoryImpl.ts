...
import type { TwitchSubscriptionRepository, TwitchSubscriptionDto } from '@sailorclawbot/contracts';
import { translatePrismaError } from './prisma-errors.js';
import { toTwitchSubscriptionDto } from './mappers.js';

export class TwitchSubscriptionRepositoryImpl implements TwitchSubscriptionRepository {
  public constructor(private readonly db: PrismaClient) {}

  public async findByGuild(guildId: string): Promise<TwitchSubscriptionDto[]> {
    const rows = await this.db.twitchSubscription.findMany({ where: { guildId } });
    return rows.map(toTwitchSubscriptionDto);
  }

  public async findByLogin(guildId: string, twitchLogin: string): Promise<TwitchSubscriptionDto | null> {
  ...
  public async updateLastStreamId(id: string, lastStreamId: string): Promise<void> {
