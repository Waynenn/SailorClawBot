...
import type { ProfileDto, ProfileRepository } from "@sailorclawbot/contracts";
...
function makeProfileRepo(
	profile: ProfileDto | null = makeProfile(),
): ProfileRepository {
	return {
		findByGuildAndUser: async () => profile,
		create: async (input) =>
			makeProfile({ guildId: input.guildId, userId: input.userId }),
		update: async (_g, _u, changes) =>
			makeProfile(changes as Partial<ProfileDto>),
