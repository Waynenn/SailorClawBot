...
import type { PermissionRepository, PermissionOverrideDto, SnowflakeId } from '@sailorclawbot/contracts';
import { PermissionService } from './PermissionService.js';
import { NotFoundError } from '../common/errors/NotFoundError.js';
import { ValidationError } from '../common/errors/ValidationError.js';

function makeOverride(id: string, guildId: SnowflakeId, userId: SnowflakeId, permission: string, allowed: boolean): PermissionOverrideDto {
...
function createRepo(store: PermissionOverrideDto[] = []): PermissionRepository {
  return {
    findByGuildUserPermission: async (guildId, userId, permission) =>
      store.find((o) => o.guildId === guildId && o.userId === userId && o.permission === permission) ?? null,
    findByGuildUser: async (guildId, userId) =>
      store.filter((o) => o.guildId === guildId && o.userId === userId),
    create: async (input) => {
      const dto: PermissionOverrideDto = { id: `ov_${store.length + 1}`, ...input };
      store.push(dto);
      return dto;
    },
    ...
    deleteByGuildAndUser: async (guildId, userId) => {
      ...
      const keep = store.filter((o) => !(o.guildId === guildId && o.userId === userId));
      store.length = 0;
      store.push(...keep);
...
test('revokePermission throws NotFoundError when override does not exist', async () => {
  const svc = new PermissionService(createRepo());
  await assert.rejects(() => svc.revokePermission('g1', 'u1', 'can_moderate'), NotFoundError);
});
...
test('hasPermission throws ValidationError for empty guildId', async () => {
  const svc = new PermissionService(createRepo());
  await assert.rejects(() => svc.hasPermission('', 'u1', 'can_moderate'), ValidationError);
});
