import type { AutoModResult, AutoModRuleDto } from "@sailorclawbot/contracts";
...
export class AutoModService {
		...
		switch (rule.type) {
			...
			case "links":
				return this.checkLinks(
					content,
					rule.config as { whitelist: string[]; action: string },
				);
			case "caps":
				return this.checkCaps(
					content,
					rule.config as { threshold: number; action: string },
				);
			case "invites":
				return this.checkInvites(
					content,
					rule.config as { whitelist: string[]; action: string },
				);
			case "mentions":
				return this.checkMentions(
					content,
					rule.config as { max: number; action: string; duration?: number },
				);
			case "words":
	...
	private checkSpam(
		...
		if (entry.count >= config.threshold) {
			...
			if (config.duration !== undefined)
				spamResult.durationMinutes = config.duration;
			return spamResult;
		}
		return null;
	}
	...
	private checkMentions(
		...
		if (mentions.length > config.max) {
			...
			if (config.duration !== undefined)
				mentionResult.durationMinutes = config.duration;
	...
	private checkWords(
		...
		for (const pattern of config.patterns) {
			...
			if (regex.test(content)) {
				...
				if (config.duration !== undefined)
					wordResult.durationMinutes = config.duration;
				return wordResult;
			}
		}
		return null;
