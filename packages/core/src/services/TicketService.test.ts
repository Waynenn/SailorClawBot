...
function createHarness(stored: TicketDto | null = null) {
	...
	const tickets: TicketRepository = {
		...
		findByChannel: async (channelId) =>
			ticket?.channelId === channelId ? ticket : null,
		listOpenByGuild: async () =>
			ticket?.status === "open" || ticket?.status === "claimed" ? [ticket] : [],
		...
		closeWithDetails: async (_id, closedById) => {
			if (!ticket) throw new Error("not found");
			ticket = { ...ticket, status: "closed", closedById };
			return ticket;
		},
		setRating: async (_id, rating) => {
			if (!ticket) throw new Error("not found");
			ticket = { ...ticket, rating };
			return ticket;
		},
		listClosedWithChannelBefore: async () =>
			ticket?.status === "closed" && ticket.channelId ? [ticket] : [],
		clearChannelId: async () => {
			if (ticket) ticket = { ...ticket, channelId: null };
		},
	};
...
test("closeTicket — throws ConflictError on already closed ticket", async () => {
	const { tickets, bus, logger } = createHarness(
		makeTicket({ status: "closed" }),
	);
	const svc = new TicketService(tickets, bus, logger);

	await assert.rejects(
		() => svc.closeTicket("ticket_1"),
		(e) => {
...
test("claimTicket — throws ConflictError on closed ticket", async () => {
	...
	await assert.rejects(
		() => svc.claimTicket("ticket_1", "staff_1"),
		(e) => {
			assert.ok(e instanceof ConflictError);
			return true;
		},
...
test("claimTicket — throws ConflictError on already claimed ticket", async () => {
	const { tickets, bus, logger } = createHarness(
		makeTicket({ status: "claimed", claimedById: "staff_1" }),
	);
	const svc = new TicketService(tickets, bus, logger);

	await assert.rejects(
		() => svc.claimTicket("ticket_1", "staff_2"),
		(e) => {
			assert.ok(e instanceof ConflictError);
			return true;
		},
