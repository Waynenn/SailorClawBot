...
export class GiveawayService {
	...
	public async create(input: CreateGiveawayInput): Promise<GiveawayDto> {
		if (input.winnersCount < 1)
			throw new ValidationError(
				"Winners count must be at least 1",
				"winnersCount",
			);
	...
	public async join(id: string, userId: string): Promise<GiveawayDto> {
		...
		if (!giveaway)
			throw new NotFoundError(`Giveaway ${id} not found`, "Giveaway");
		if (giveaway.endedAt)
			throw new ValidationError("This giveaway has already ended", "id");
		if (giveaway.endsAt < new Date())
			throw new ValidationError("This giveaway has already ended", "id");
		if (giveaway.participants.includes(userId))
			throw new ConflictError("Already entered this giveaway", "userId");
		return this.repo.addParticipant(id, userId);
	}

	public async end(
		id: string,
	): Promise<{ giveaway: GiveawayDto; winners: string[] }> {
		const giveaway = await this.repo.findById(id);
		if (!giveaway)
			throw new NotFoundError(`Giveaway ${id} not found`, "Giveaway");
	...
	public async reroll(
		...
		if (!giveaway)
			throw new NotFoundError(`Giveaway ${id} not found`, "Giveaway");
		if (!giveaway.endedAt)
			throw new ValidationError("Giveaway has not ended yet", "id");
		const winners = pickWinners(giveaway.participants, giveaway.winnersCount);
		const updated = await this.repo.end(id, winners);
		return { giveaway: updated, winners };
	}
