...
export class ShopService {
  ...
  public async buyItem(
    ...
    if (!item || item.guildId !== guildId) throw new NotFoundError('Item', itemId);
    ...
    // Atomically decrement stock first — prevents overselling when concurrent buyers race.
    ...
    if (item.stock !== null) {
      ...
      if (!available) throw new ConflictError('Item is out of stock', 'OUT_OF_STOCK');
    }

    const updatedWallet = await this.wallets.adjustBalance(wallet.id, -totalPaid);
    const inventoryItem = await this.inventory.addItem(guildId, userId, itemId);
    await this.transactions.create({ walletId: wallet.id, amount: -totalPaid, reason: `Buy: ${item.name}` });

    this.logger.info('Item bought', { guildId, userId, itemId, totalPaid: totalPaid.toString() });

    return { wallet: { balance: updatedWallet.balance }, item, totalPaid, inventoryItem };
  }
  ...
  public async sellItem(
    ...
    const invItem = await this.inventory.findByUserAndItem(guildId, userId, itemId);
    if (!invItem) throw new NotFoundError('InventoryItem', `${guildId}:${userId}:${itemId}`);
  ...
  public async createItem(guildId: SnowflakeId, data: Omit<CreateItemDto, 'guildId'>): Promise<ItemDto> {
