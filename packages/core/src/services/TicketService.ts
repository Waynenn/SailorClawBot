...
export class TicketService {
  ...
  public async openTicket(
    ...
    const ticket = await this.tickets.create({ guildId, openedByUserId, channelId, subject });
  ...
  public async claimTicket(id: string, claimedById: SnowflakeId): Promise<TicketDto> {
    const existing = await this.tickets.findById(id);
    if (!existing) throw new NotFoundError('Ticket', id);
    if (existing.status === 'closed') throw new ConflictError('Ticket is closed', 'TICKET_ALREADY_CLOSED');
    if (existing.status === 'claimed') throw new ConflictError('Ticket already claimed', 'TICKET_ALREADY_CLAIMED');
  ...
  public async closeTicket(id: string): Promise<TicketDto> {
    const existing = await this.tickets.findById(id);
    if (!existing) throw new NotFoundError('Ticket', id);
    if (existing.status === 'closed') throw new ConflictError('Ticket is already closed', 'TICKET_ALREADY_CLOSED');
  ...
  public async closeTicketByUser(id: string, closedById: SnowflakeId): Promise<TicketDto> {
    const existing = await this.tickets.findById(id);
    if (!existing) throw new NotFoundError('Ticket', id);
    if (existing.status === 'closed') throw new ConflictError('Ticket is already closed', 'TICKET_ALREADY_CLOSED');
    const ticket = await this.tickets.closeWithDetails(id, closedById);
    this.logger.info('Ticket closed', { ticketId: id, closedById, guildId: ticket.guildId });
    await this.bus.publish({
      name: EventNames.TicketClosed,
      payload: { ticketId: id, guildId: ticket.guildId },
      occurredAt: new Date(),
