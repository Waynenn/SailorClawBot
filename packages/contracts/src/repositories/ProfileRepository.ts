...
export interface ProfileRepository {
  ...
  updateXp(guildId: SnowflakeId, userId: SnowflakeId, data: { xp: number; level: number; totalXp: number }): Promise<ProfileDto>;
  findLeaderboard(guildId: SnowflakeId, skip: number, take: number): Promise<ProfileDto[]>;
