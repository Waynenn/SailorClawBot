...
export interface TwitchSubscriptionRepository {
  ...
  findByLogin(guildId: string, twitchLogin: string): Promise<TwitchSubscriptionDto | null>;
  findAll(): Promise<TwitchSubscriptionDto[]>;
  create(data: { guildId: string; twitchLogin: string; notifyChannelId: string; mentionRoleId?: string; customMessage?: string }): Promise<TwitchSubscriptionDto>;
