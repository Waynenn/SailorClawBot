...
export async function removeGuildBan(
  ...
  try {
    await rest.delete(Routes.guildBan(guildId, userId), { reason: 'Temp-ban expired' });
    return 'removed';
  } catch (error) {
...
export async function removeMemberRole(
  ...
  try {
    await rest.delete(Routes.guildMemberRole(guildId, userId, roleId), { reason: 'Mute expired' });
  } catch (error) {
...
export async function postMessage(
  ...
  return (await rest.post(Routes.channelMessages(channelId), { body })) as { id: string };
}
