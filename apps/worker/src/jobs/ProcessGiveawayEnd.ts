...
import { dmUser, editMessage, postMessage } from "../lib/discord.js";
	...
	for (const g of expired) {
		try {
			...
			const winnerMentions =
				winners.length > 0 ? winners.map((id) => `<@${id}>`).join(", ") : null;
			...
			if (g.messageId) {
				await editMessage(c.rest, g.channelId, g.messageId, {
					...
					c.logger.warn("Failed to edit giveaway message", {
						id: g.id,
						err: String(err),
					}),
				);
			...
			await postMessage(c.rest, g.channelId, { content: announcement }).catch(
				(err) =>
					c.logger.warn("Failed to announce giveaway winners", {
						id: g.id,
						err: String(err),
					}),
			);
			...
			c.logger.error("Failed to end giveaway", {
				id: g.id,
				guildId: g.guildId,
				error: String(error),
			});
