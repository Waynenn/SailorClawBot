...
export async function processBanExpiry(c: Container): Promise<void> {
  ...
  for (const ban of expired) {
    try {
      ...
      }).catch((err) => c.logger.warn('Mod-log failed for ban expiry', { id: ban.id, err: String(err) }));
    } catch (error) {
      c.logger.error('Failed to process ban expiry', { id: ban.id, guildId: ban.guildId, error: String(error) });
    }
  }
}
