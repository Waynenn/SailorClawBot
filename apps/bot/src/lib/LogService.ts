 ...
 */
export function resolveLogChannelId(settings: GuildSettingsDto, entry: LogEntry): string | null {
  // Event-type gate (empty filter = log everything)
  if (settings.logEvents.length > 0 && !settings.logEvents.includes(entry.event)) return null;
  // Ignored source channel (e.g. spam channel)
  if (entry.channelId && settings.logIgnoredChannels.includes(entry.channelId)) return null;
  // Per-event override → fallback channel
  return settings.logChannelOverrides[entry.event] ?? settings.logChannelId ?? null;
}

/** Build and dispatch a log embed to the resolved channel. Best-effort: never throws. */
export async function sendLog(client: Client, settings: GuildSettingsDto, entry: LogEntry): Promise<void> {
  ...
  if (entry.target) embed.addFields({ name: 'User', value: `<@${entry.target}>`, inline: true });
  if (entry.caseId) embed.addFields({ name: 'Case', value: `#${entry.caseId}`, inline: true });
  if (entry.reason) embed.addFields({ name: 'Reason', value: entry.reason.slice(0, 1024) });
  if (entry.fields?.length) embed.addFields(entry.fields.map((f) => ({ ...f, value: f.value.slice(0, 1024) })));
