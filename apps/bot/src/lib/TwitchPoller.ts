...
export class TwitchPoller {
  ...
  public start(): void {
    this.poll().catch((e) => this.logger.error('TwitchPoller initial poll failed', { error: String(e) }));
    this.timer = setInterval(() => {
      this.poll().catch((e) => this.logger.error('TwitchPoller poll failed', { error: String(e) }));
    }, POLL_INTERVAL_MS);
  }
  ...
  private async getToken(): Promise<string> {
    ...
    const res = await fetch(`${TWITCH_AUTH}?client_id=${this.clientId}&client_secret=${this.clientSecret}&grant_type=client_credentials`, {
      method: 'POST',
    });
  ...
  private async sendNotification(
    sub: { notifyChannelId: string; mentionRoleId: string | null; customMessage: string | null; twitchLogin: string },
    stream: TwitchStream
  ): Promise<void> {
    const channel = await this.client.channels.fetch(sub.notifyChannelId).catch(() => null);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Игра', value: stream.game_name || 'Unknown', inline: true },
      )
      .setImage(thumbnail)
    ...
    await (channel as import('discord.js').TextChannel).send({
      ...
      embeds: [embed],
    }).catch((e) => this.logger.error('Failed to send Twitch notification', { error: String(e) }));
  }
}
