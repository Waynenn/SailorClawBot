...
export function startTicketCleaner(client: Client, container: Container): void {
	const run = async () => {
		...
		const expired = await container.ticketService
			.listExpiredChannels(cutoff)
			.catch(() => []);
...
export async function updateStatsEmbed(
	guild: Guild,
	container: Container,
): Promise<void> {
	...
	const channel = await guild.channels
		.fetch(settings.ticketChannelId)
		.catch(() => null);
	if (!channel?.isTextBased()) return;

	const msg = await (channel as TextChannel).messages
		.fetch(settings.ticketStatsMessageId)
		.catch(() => null);
