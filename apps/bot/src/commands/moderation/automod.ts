...
export const automodCommand: Command = {
	...
	async execute(
		...
		if (sub === "enable") {
			...
			await container.autoModRepo.upsert(
				guildId,
				type,
				true,
			...
			await interaction.reply({
				content: `✅ Rule **${type}** enabled with default config.`,
				ephemeral: true,
			});
			return;
		}
		...
		if (sub === "disable") {
			...
			if (!existing) {
				await interaction.reply({
					content: `⚠️ Rule **${type}** is not configured yet.`,
					ephemeral: true,
				});
				return;
			}
		...
		if (sub === "verify") {
			...
			if (enabled) {
				...
				const embed = new EmbedBuilder()
					...
					.setDescription(
						`Click the button below to get the <@&${role.id}> role and access the server.`,
					);
