...
export const muteCommand: Command = {
	...
	async execute(
			...
			await interaction.editReply({
				embeds: [
					new EmbedBuilder()
						...
						.setDescription("🚫 You lack permission to mute members."),
				],
			});
			return;
		}
		...
		try {
			const mute = await container.moderationService.muteUser(
				guildId,
				target.id,
				duration,
				moderatorId,
			...
			if (targetMember) {
				await targetMember
					.timeout(duration * 60_000, reason ?? `[Case #${mute.caseNumber}]`)
					.catch(() => null);
			}

			const until =
				mute.expiresAt instanceof Date
