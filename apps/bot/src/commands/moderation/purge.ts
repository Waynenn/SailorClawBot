...
export const purgeCommand: Command = {
	data: new SlashCommandBuilder()
		...
		.addBooleanOption((o) =>
			o.setName("bots").setDescription("Only delete bot messages"),
	...
	async execute(
		...
		if (contains)
			filtered = filtered.filter((m) => m.content.includes(contains));
		if (botsOnly) filtered = filtered.filter((m) => m.author.bot);
		filtered = filtered.slice(0, count);

		if (filtered.length === 0) {
			await interaction.editReply(
				"No eligible messages found (messages must be < 14 days old).",
			);
			return;
		}
