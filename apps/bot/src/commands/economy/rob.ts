...
export const robCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction, container: Container): Promise<void> {
    ...
    try {
      ...
      const { stolen, backfired, fined, wallet } = await container.economyService.rob(
      ...
      if (backfired) {
        embed
          ...
          .setDescription(`You tried to rob ${target} but got caught!\nYou were fined **${fined.toLocaleString()} coins**.`)
          .addFields({ name: 'Your Balance', value: `${wallet.balance.toLocaleString()} coins`, inline: true });
      } else {
        embed
          .setColor(EMBED_COLORS.economy)
          .setTitle('Successful Rob!')
          .setDescription(`You robbed ${target} and stole **${stolen.toLocaleString()} coins**!`)
          .addFields({ name: 'Your Balance', value: `${wallet.balance.toLocaleString()} coins`, inline: true });
      }
