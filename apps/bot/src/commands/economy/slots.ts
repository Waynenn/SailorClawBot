...
export const slotsCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction, container: Container): Promise<void> {
    ...
    try {
      ...
      const { reels, multiplier, won, payout, wallet } = await container.economyService.slots(guildId, userId, amount, {
        ...
        .addFields(
          {
            ...
            value: won ? `+${(payout - amount).toLocaleString()} coins` : `-${amount.toLocaleString()} coins`,
            inline: true,
          },
