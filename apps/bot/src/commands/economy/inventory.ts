...
export const inventoryCommand: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction, container: Container): Promise<void> {
    ...
    try {
      const items = await container.inventoryService.listInventory(guildId, userId);

      if (items.length === 0) {
        await interaction.editReply('Your inventory is empty. Visit the `/shop` to buy items!');
        return;
      }
