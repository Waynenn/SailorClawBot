	...
	async execute(
		...
		try {
			if (sub === "setup") {
				...
				if (
					...
					await interaction.editReply(
						"You need **Manage Guild** permission to set up tickets.",
					);
					return;
				}
				...
				if (!(categoryOption instanceof CategoryChannel)) {
					await interaction.editReply(
						"Please select a **category channel** for tickets.",
					);
					return;
				}
				...
				const statsMsg = await (interaction.channel as TextChannel).send({
					embeds: [statsEmbed],
				});
				...
				await interaction.editReply(
					`✅ Ticket system set up in <#${interaction.channelId}>. Category: **${categoryOption.name}**.`,
				);
				return;
			}
			...
			if (sub === "close") {
				...
				const ticket = await container.ticketService.findByChannel(
				...
				await interaction.editReply(
					`🔒 Ticket closed${reason ? ` — ${reason}` : ""}. Channel will be deleted in 7 days.`,
				);
			...
			if (sub === "add") {
				...
				await (interaction.channel as TextChannel).permissionOverwrites.create(
					target.id,
					{
						ViewChannel: true,
						SendMessages: true,
			...
			if (sub === "remove") {
				...
				await (interaction.channel as TextChannel).permissionOverwrites.delete(
					target.id,
				);
				await interaction.editReply(
					`✅ Removed <@${target.id}> from the ticket.`,
				);
				return;
			}
		} catch (error) {
