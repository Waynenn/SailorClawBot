  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName('set')
        ...
        .addStringOption((o) => o.setName('message').setDescription('Template: {username} {mention} {server} {memberCount} {date}').setRequired(true))
    )
    .addSubcommand((s) =>
      s.setName('leave')
    ...
    .addSubcommand((s) => s.setName('disable').setDescription('Disable welcome and leave messages')),
  ...
  async execute(interaction: ChatInputCommandInteraction, container: Container): Promise<void> {
    ...
    try {
      if (sub === 'set') {
        ...
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.info).setDescription('✅ Welcome and leave messages disabled.')],
        });
      }
    } catch (error) {
