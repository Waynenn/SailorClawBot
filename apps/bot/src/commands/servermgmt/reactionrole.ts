  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName('add')
        ...
        .addChannelOption((o) => o.setName('channel').setDescription('Channel with the message').setRequired(true))
  ...
  async execute(interaction: ChatInputCommandInteraction, container: Container): Promise<void> {
    ...
    try {
      if (sub === 'add') {
        ...
        await container.reactionRoleRepo.create({ guildId, messageId, channelId: channel.id, emoji, roleId: role.id });
        ...
        const messageId = interaction.options.getString('message_id', true);
        const emoji = interaction.options.getString('emoji', true);
        await container.reactionRoleRepo.deleteByMessageAndEmoji(guildId, messageId, emoji);
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.info)
            .setDescription(`✅ Reaction role for ${emoji} on message \`${messageId}\` removed.`)],
        });
      } else {
        const messageId = interaction.options.getString('message_id', true);
        const entries = await container.reactionRoleRepo.findByMessage(guildId, messageId);
        if (entries.length === 0) {
          await interaction.editReply({ content: 'No reaction roles on that message.' });
          return;
        }
