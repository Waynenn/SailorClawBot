  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName('create')
        ...
        .addStringOption((o) => o.setName('duration').setDescription('Duration: 1m, 2h, 3d').setRequired(true))
    ...
    try {
      if (sub === 'create') {
        ...
        if (durationMs < 60_000) {
          await interaction.editReply({ content: 'Duration must be at least 1 minute (e.g. `1m`, `2h`, `1d`).' });
          return;
        }
        ...
        const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
          new ButtonBuilder().setCustomId(`giveaway_join_${giveaway.id}`).setLabel('🎉 Enter').setStyle(ButtonStyle.Primary)
        );
        ...
        const winnerMentions = winners.length > 0 ? winners.map((w) => `<@${w}>`).join(', ') : 'No participants';
        const channel = await interaction.guild?.channels.fetch(giveaway.channelId).catch(() => null);
        if (channel?.isTextBased()) {
          await channel.send({ content: `🎉 **${giveaway.prize}** winners: ${winnerMentions}` }).catch(() => null);
        }
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.giveaway).setTitle('Giveaway Ended').setDescription(`Winners: ${winnerMentions}`)],
        });

      } else if (sub === 'reroll') {
        const id = interaction.options.getString('id', true);
        const { giveaway, winners } = await container.giveawayService.reroll(id);
        const winnerMentions = winners.length > 0 ? winners.map((w) => `<@${w}>`).join(', ') : 'No participants';
        const channel = await interaction.guild?.channels.fetch(giveaway.channelId).catch(() => null);
        if (channel?.isTextBased()) {
          await channel.send({ content: `🔁 **Reroll!** New winners for **${giveaway.prize}**: ${winnerMentions}` }).catch(() => null);
        }
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.giveaway).setTitle('Rerolled').setDescription(`New winners: ${winnerMentions}`)],
        });
        ...
        const lines = active.map((g) => `• **${g.prize}** — ends <t:${Math.floor(g.endsAt.getTime() / 1000)}:R> (${g.participants.length} entries) \`${g.id}\``);
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.giveaway).setTitle('Active Giveaways').setDescription(lines.join('\n'))],
        });
      }
    } catch (error) {
