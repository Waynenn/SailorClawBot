  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName('setup')
        ...
        .addIntegerOption((o) => o.setName('threshold').setDescription('Stars needed (default: 3)').setMinValue(1).setMaxValue(100))
    )
    ...
    try {
      if (sub === 'setup') {
        ...
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.starboard)
        ...
        await container.guildSettingsRepo.upsert(guildId, { starboardEnabled: false, starboardChannelId: null });
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor(EMBED_COLORS.info).setDescription('⭐ Starboard disabled.')],
        });
      }
    } catch (error) {
