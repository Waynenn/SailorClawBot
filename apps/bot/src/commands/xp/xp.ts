...
export const xpCommand: Command = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((s) =>
			s
				...
				.addUserOption((o) =>
					o.setName("user").setDescription("Пользователь").setRequired(true),
				)
				.addIntegerOption((o) =>
					o
						.setName("amount")
	...
	async execute(interaction, container) {
		...
		if (sub === "give") {
			const { profile } = await container.xpService.grantXp(
				guildId,
				target.id,
				amount,
			);
			const embed = new EmbedBuilder()
				...
				.setDescription(
					`✅ Выдано **${amount} XP** для <@${target.id}>. Уровень: **${profile.level}**, Всего XP: **${profile.totalXp}**`,
				);
			await interaction.editReply({ embeds: [embed] });
		} else {
			const profile = await container.xpService.setXp(
				guildId,
				target.id,
				amount,
			);
			const embed = new EmbedBuilder()
				.setColor(EMBED_COLORS.xp)
				.setDescription(
					`✅ XP для <@${target.id}> установлен на **${amount}**. Уровень: **${profile.level}**`,
				);
			await interaction.editReply({ embeds: [embed] });
		}
	},
