...
export const twitchCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        ...
        .addStringOption((o) => o.setName('login').setDescription('Логин Twitch-канала').setRequired(true))
        .addChannelOption((o) => o.setName('channel').setDescription('Discord-канал для уведомлений').setRequired(true))
        .addRoleOption((o) => o.setName('role').setDescription('Роль для пинга (необязательно)'))
        .addStringOption((o) => o.setName('message').setDescription('Шаблон сообщения: {streamer}, {title}, {game}, {url}'))
    )
    .addSubcommand((s) =>
      s
        .setName('remove')
  ...
  async execute(interaction, container) {
    ...
    if (sub === 'add') {
      ...
      const existing = await container.twitchSubRepo.findByLogin(guildId, login);
      if (existing) {
        await interaction.editReply({ content: `❌ Канал \`${login}\` уже отслеживается.` });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`✅ Twitch-канал **${login}** добавлен. Уведомления → <#${discordChannel.id}>${role ? ` (пинг ${role})` : ''}`);
      await interaction.editReply({ embeds: [embed] });
    } else if (sub === 'remove') {
      const login = interaction.options.getString('login', true).toLowerCase();
      const sub_ = await container.twitchSubRepo.findByLogin(guildId, login);
      if (!sub_) {
        await interaction.editReply({ content: `❌ Канал \`${login}\` не найден.` });
        return;
      }
      ...
      if (subs.length === 0) {
        await interaction.editReply({ content: 'Нет отслеживаемых Twitch-каналов.' });
        return;
      }
      const lines = subs.map((s) => `• **${s.twitchLogin}** → <#${s.notifyChannelId}>${s.mentionRoleId ? ` <@&${s.mentionRoleId}>` : ''}`);
      const embed = new EmbedBuilder()
        .setColor(EMBED_COLORS.info)
