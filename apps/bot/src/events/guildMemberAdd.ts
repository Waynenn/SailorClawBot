...
async function sendRaidAlert(
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(
			`**${joinsPerMin} members** joined in the last 60 seconds. Consider enabling verification.`,
		)
		.setTimestamp();
	...
	client.on("guildMemberAdd", async (member: GuildMember) => {
		...
		if (settings.minAccountAgeDays > 0) {
			...
			if (ageDays < settings.minAccountAgeDays) {
				await member
					.kick(
						`Account too new (${Math.floor(ageDays)}d, min ${settings.minAccountAgeDays}d)`,
					)
					.catch(() => null);
				return;
			}
		...
		if (
			settings.raidJoinsPerMinute > 0 &&
			joinsPerMin >= settings.raidJoinsPerMinute
		) {
			const now = Date.now();
