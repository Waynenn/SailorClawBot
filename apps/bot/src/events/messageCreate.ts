...
async function executeAutoModAction(
	...
	try {
		if (result.action === "warn") {
			await container.moderationService.warnUser(
				guildId,
				userId,
				reason,
				BOT_ID,
			...
			await container.moderationService.muteUser(
				guildId,
				userId,
				duration,
				BOT_ID,
		...
		if (logChannel?.isTextBased()) {
			const embed = new EmbedBuilder()
				...
				.addFields(
					...
					{
						name: "Content",
						value: message.content.slice(0, 300) || "(empty)",
					},
				)
				.setTimestamp();
...
async function handleTicketOpen(
	...
	const settings = await container.guildSettingsRepo.findByGuild(
		message.guildId!,
	);
	if (
	...
	const ticket = await container.ticketService.openTicket(
		guild.id,
		message.author.id,
		ticketChannel.id,
		subject,
...
export function registerMessageCreateHandler(
	...
	client.on("messageCreate", async (message: Message) => {
		...
		const channelExcluded = await container.noXpTargetRepo.isExcluded(
			guildId,
			message.channelId,
		);
		if (channelExcluded) {
			xpCooldowns.set(key, prevStamp);
			return;
		}

		const memberRoleIds = message.member?.roles.cache.map((r) => r.id) ?? [];
		for (const roleId of memberRoleIds) {
			if (await container.noXpTargetRepo.isExcluded(guildId, roleId)) {
				xpCooldowns.set(key, prevStamp);
				return;
			}
		}
		...
		let multiplier = 1.0;
		const channelMult = await container.xpMultiplierRepo.findByTarget(
			guildId,
			message.channelId,
			"channel",
		);
		...
		for (const roleId of memberRoleIds) {
			const roleMult = await container.xpMultiplierRepo.findByTarget(
				guildId,
				roleId,
				"role",
			);
			if (roleMult && roleMult.multiplier > multiplier)
		...
		const { leveled, newLevel, profile } = await container.xpService.grantXp(
			guildId,
			userId,
			amount,
		);
		...
		const levelRole = await container.levelRoleRepo.findByLevel(
			guildId,
			newLevel,
		);
		...
		const template =
			...
			"GG {mention}, you reached level **{level}**!";
		...
		const sendChannel = ((rawTarget?.isTextBased() ? rawTarget : null) ??
