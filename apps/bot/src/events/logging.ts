...
import { type LogEntry, sendLog } from "../lib/LogService.js";
...
export function registerLoggingHandlers(
	...
	client.on("messageDelete", async (message: Message | PartialMessage) => {
		...
		const content =
			cached?.content ?? message.content ?? "*(content unavailable)*";
	...
	client.on(
		...
		async (
			...
			if (cached)
				await container.messageCache.set({ ...cached, content: after });
		},
	);
	...
	client.on(
		...
		async (member: GuildMember | PartialGuildMember) => {
			...
			await emit(client, container, member.guild.id, {
				...
				fields:
					roles.length > 0
						? [{ name: "Roles", value: roles.join(", ").slice(0, 1024) }]
						: undefined,
			});
		},
	);
		...
		async (
			...
			if (oldRoles) {
				...
				if (added.length > 0)
					fields.push({
						name: "Roles added",
						value: added.join(", ").slice(0, 1024),
					});
				if (removed.length > 0)
					fields.push({
						name: "Roles removed",
						value: removed.join(", ").slice(0, 1024),
					});
			}
			...
			if (!oldState.channelId && newState.channelId) {
				...
				await emit(client, container, guildId, {
					...
					fields: [
						{
							name: "Channel",
							value: `<#${oldState.channelId}> → <#${newState.channelId}>`,
						},
					],
				});
			}
